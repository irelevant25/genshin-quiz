const SITES_TOP_DAILY_COMPONENT = {
    VUE_COMPONENT: Vue.createApp({
        ...
        methods: {
            ...
            renderQuiz(quizName) {
                ...
                if (quizName === 'banners') {
                    this.generateCompnent(SITES_TOP_QUIZZES_BANNERS_COMPONENT.VUE_COMPONENT);
                } else if (quizName === 'pixelate') {
                    this.generateCompnent(SITES_TOP_QUIZZES_PIXELATE_COMPONENT.VUE_COMPONENT);
                } else if (quizName === 'mismatch') {
                    this.generateCompnent(SITES_TOP_QUIZZES_MISMATCH_COMPONENT.VUE_COMPONENT);
                } else if (quizName === 'music') {
                    this.generateCompnent(SITES_TOP_QUIZZES_MUSIC_COMPONENT.VUE_COMPONENT);
                } else if (quizName === 'dish') {
                    this.generateCompnent(SITES_TOP_QUIZZES_DISH_COMPONENT.VUE_COMPONENT);
                } else if (quizName === 'voice') {
                    this.generateCompnent(SITES_TOP_QUIZZES_VOICE_COMPONENT.VUE_COMPONENT);
                }
            },
    ...
    onShow() {
        document.querySelector(`#${MENU_ITEMS_TOP.daily.id}`).classList.remove('d-none');
    },

    onHide() {
        document.querySelector(`#${MENU_ITEMS_TOP.daily.id}`).classList.add('d-none');
    },
};

document.addEventListener('DOMContentLoaded', () => {
    SITES_TOP_DAILY_COMPONENT.VUE_COMPONENT.mount(`#${MENU_ITEMS_TOP.daily.id}`);
});
