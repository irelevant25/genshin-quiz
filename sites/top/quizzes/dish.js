const SITES_TOP_QUIZZES_DISH_COMPONENT = {
    VUE_COMPONENT: {
        props: {
            daily: {
                type: Boolean,
    ...
    onShow() {
        document.querySelector(`#${QUIZZES.dish.id}`).classList.remove('d-none');
    },

    onHide() {
        document.querySelector(`#${QUIZZES.dish.id}`).classList.add('d-none');
    },
};
...
document.addEventListener('DOMContentLoaded', () => {
    const quiz = Vue.createApp({
        components: {
            'base-component': createComponent(SITES_TOP_QUIZZES_DISH_COMPONENT.VUE_COMPONENT, {}),
        },
