const SITES_TOP_QUIZZES_VOICE_COMPONENT = {
    VUE_COMPONENT: {
        props: {
            daily: {
                type: Boolean,
    ...
    onShow() {
        document.querySelector(`#${QUIZZES.voice.id}`).classList.remove('d-none');
    },

    onHide() {
        document.querySelector(`#${QUIZZES.voice.id}`).classList.add('d-none');
    },
};
...
document.addEventListener('DOMContentLoaded', () => {
    const quiz = Vue.createApp({
        components: {
            'base-component': createComponent(SITES_TOP_QUIZZES_VOICE_COMPONENT.VUE_COMPONENT, {}),
        },
