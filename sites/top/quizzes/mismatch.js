const SITES_TOP_QUIZZES_MISMATCH_COMPONENT = {
    VUE_COMPONENT: {
        props: {
            daily: {
                type: Boolean,
    ...
    onShow() {
        document.querySelector(`#${QUIZZES.mismatch.id}`).classList.remove('d-none');
    },

    onHide() {
        document.querySelector(`#${QUIZZES.mismatch.id}`).classList.add('d-none');
    },
};
...
document.addEventListener('DOMContentLoaded', () => {
    const quiz = Vue.createApp({
        components: {
            'base-component': createComponent(SITES_TOP_QUIZZES_MISMATCH_COMPONENT.VUE_COMPONENT, {}),
        },
