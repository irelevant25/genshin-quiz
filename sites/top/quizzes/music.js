const SITES_TOP_QUIZZES_MUSIC_COMPONENT = {
    VUE_COMPONENT: {
        props: {
            daily: {
                type: Boolean,
        ...
        methods: {
            ...
            resetQuiz() {
                Object.assign(this, {
                    ...
                    questionEntity: getRandomCharacter((x) => x.demo_music !== null),
            ...
            refreshPlayerTime() {
                ...
                this.startTime = 0;
                this.endTime = this.isQuestionComplete ? audio.duration : this.triesEffects.find((x) => x.try === this.tries.length)?.data ?? audio.duration;
    ...
    onShow() {
        document.querySelector(`#${QUIZZES.music.id}`).classList.remove('d-none');
    },

    onHide() {
        document.querySelector(`#${QUIZZES.music.id}`).classList.add('d-none');
    },
};
...
document.addEventListener('DOMContentLoaded', () => {
    const quiz = Vue.createApp({
        components: {
            'base-component': createComponent(SITES_TOP_QUIZZES_MUSIC_COMPONENT.VUE_COMPONENT, {}),
        },
